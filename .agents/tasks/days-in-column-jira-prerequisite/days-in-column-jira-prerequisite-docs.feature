Feature: Days in Column — Jira prerequisite in user documentation
  ...
  Scenario: S1 — EN reader learns the Jira prerequisite from Prerequisites
    ...
    Then the section states the feature works only if Jira's "Show days in column" is enabled for the board
    And the section includes the UI path "Board configuration → Card layout → Show days in column"

  @SC-DOC-EN-2
  Scenario: EN How to configure reminds about the Jira board setting
    Given the English Days in Column user guide at "website/docs/features/card-information/days-in-column.md"
    When the reader reads the "How to configure" section
    Then a bullet mentions the Jira board setting "Show days in column"
    And the bullet explains that without this setting the jira-helper badge cannot work correctly
    And the bullet includes the path "Board configuration → Card layout"
  ...
  Scenario: S2 — RU reader learns the Jira prerequisite from Требования
    ...
    Then the section states the feature works only if Jira's "Показывать дни в колонке" is enabled for the board
    And the section includes the UI path "Настройки доски → Макет карточки → Показывать дни в колонке"

  @SC-DOC-RU-2
  Scenario: RU Как настроить reminds about the Jira board setting
    Given the Russian Days in Column user guide at "website/i18n/ru/docusaurus-plugin-content-docs/current/features/card-information/days-in-column.md"
    When the reader reads the "Как настроить" section
    Then a bullet mentions the Jira board setting "Показывать дни в колонке"
    And the bullet explains that without this setting the jira-helper badge cannot work correctly
    And the bullet includes the path "Настройки доски → Макет карточки"
  ...
  Scenario: S3 — EN Troubleshooting links missing badge to disabled Jira setting
    ...
    Then the "Badge missing" item mentions that Jira "Show days in column" must be enabled
    And the item includes the path "Board configuration → Card layout"

  @SC-DOC-RU-3
  Scenario: S3 — RU Troubleshooting links missing badge to disabled Jira setting
    Given the Russian Days in Column user guide at "website/i18n/ru/docusaurus-plugin-content-docs/current/features/card-information/days-in-column.md"
    When the reader reads the "Troubleshooting" section
    Then an item about a missing badge mentions that Jira "Показывать дни в колонке" must be enabled
    And the item includes the path "Настройки доски → Макет карточки"
  ...
  Scenario: Documentation wording matches the extension Alert (docs ↔ UI parity)
    ...
    Then the EN setting label is "Show days in column"
    And the RU setting label is "Показывать дни в колонке"
    And the EN UI path matches "Board configuration → Card layout → Show days in column"
    And the RU UI path matches "Настройки доски → Макет карточки → Показывать дни в колонке"
