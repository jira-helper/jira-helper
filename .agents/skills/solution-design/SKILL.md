...
Если в дизайне есть React Container — также прочитай `docs/component-containers.md` и явно зафиксируй ownership доменной логики.
...
[Карта ответственности: какие доменные вычисления живут в Model, какие в pure utils, какие данные Container только прокидывает во View. Для каждого Container явно указать: UI-only state, model commands, запрещённые domain derivations.]
...
Обязательная секция для фич с React Container.

Для каждого business rule укажи владельца:

- persisted/cascade state → Settings/Property Model;
- runtime derived state, counters, warning lists → Runtime/Data Model;
- stateless transformation → `utils/` pure function;
- UI-only state → Container;
- presentation → View.

Container не должен владеть доменными вычислениями. Если дизайн предполагает `useMemo` / `useEffect` в Container, опиши почему это UI orchestration, а не business logic. Подробные правила — `docs/component-containers.md`.
...
6. **Logic Ownership обязателен** — Container не получает доменную логику
7. **Migration Plan реалистичен** — самодостаточные фазы
...
- [ ] Logic Ownership (Model / utils / Container / View; нет бизнес-логики в Container)
