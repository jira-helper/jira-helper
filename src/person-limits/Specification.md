...
# ФИЧА: Board Page (Применение лимитов)

## User Story

Я как участник команды хочу видеть на доске индикацию загрузки каждого человека согласно установленным лимитам, чтобы понимать, кто перегружен.

## Логика применения

### 1. Подсчет задач
Для каждого лимита (`PersonLimit`) считаются задачи, которые:
- Назначены на этого пользователя (`assignee`)
- Находятся в одной из указанных колонок (`columns`). Если список пуст — во всех колонках.
- Находятся в одном из указанных свимлейнов (`swimlanes`). Если список пуст — во всех свимлейнах.
- Имеют тип задачи из списка `includedIssueTypes`. Если список пуст или не задан — считаются все типы задач.

### 2. Визуализация
- **Аватары в шапке**: В верхней панели доски (рядом с названием) отображаются аватары пользователей, для которых заданы лимиты.
- **Счетчик**: Под/рядом с аватаром отображается `текущее_кол-во / лимит`.
- **Цветовая индикация**:
    - Зеленый: `текущее < лимит`
    - Желтый: `текущее == лимит`
    - Красный: `текущее > лимит`
- **Подсветка карточек**: Если лимит превышен, все карточки этого пользователя, входящие в подсчет данного лимита, подсвечиваются красным фоном.

### 3. Фильтрация по клику
- При клике на аватар в шапке доски включается режим "View only this person's limited cards".
- На доске остаются видимыми только те карточки, которые входят в подсчет лимита этого пользователя.
- Остальные карточки, пустые свимлейны и группы скрываются.
- Повторный клик отключает фильтр.
