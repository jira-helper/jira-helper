...
# Cypress BDD Testing (Component)

## Назначение

Автоматическое выполнение сценариев из `.feature` файлов через Cypress component tests.
...
import { mountSettingsButton, columns, swimlanes } from '../helpers';

// --- Given steps ---

Given(
  /^a limit: login "([^"]*)" name "([^"]*)" value (\d+) columns "([^"]*)" swimlanes "([^"]*)" issueTypes "([^"]*)"$/,
  (login, displayName, value, columnsStr, swimlanesStr, issueTypesStr) => {
    // Создание лимита в property store
  }
);
...
When('I open the settings modal', () => {
  mountSettingsButton();  // Монтирует реальный SettingsButtonContainer
  cy.contains('button', 'Manage per-person WIP-limits').click();
});
